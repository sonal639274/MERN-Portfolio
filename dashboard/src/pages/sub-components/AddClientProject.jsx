...
const AddClientProject = () => {
  ...
  const { loading, error, message } = useSelector((state) => state.clientproject);
  const dispatch = useDispatch();
  const handleAddNewClientProject = (e) => {
    e.preventDefault();
    const formData = new FormData();
    formData.append("title", title);
    formData.append("projectLink", projectLink);
    formData.append("projectBanner", projectBanner);
    dispatch(addNewClientProject(formData));
  };
  useEffect(() => {
    if (error) {
      toast.error(error);
  ...
  return (
    <>
      <div className="padding flex mt-7 justify-center items-center min-h-[100vh] sm:gap-4 sm:py-4 sm:pl-14">
        <form
          onSubmit={handleAddNewClientProject}
